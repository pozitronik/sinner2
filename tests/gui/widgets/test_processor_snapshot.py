...
def test_to_batch_task_builds_task_from_snapshot_and_batch_inputs(widget, tmp_path):
    """to_batch_task fills the processor fields from the snapshot and forwards
    the batch-only inputs (paths, format, cleanup, execution profiles) as given."""
    from sinner2.batch.task import BatchCleanupMode, BatchOutputFormat
    ...
    tgt.write_bytes(b"x")
    swap_exec = OnnxExecution(workers=4, providers=["CPUExecutionProvider"])
    enh_exec = TorchExecution(workers=2, device="cpu")
    up_exec = TorchExecution(workers=1, device="cpu")
    task = s.to_batch_task(
        source_path=src,
        target_path=tgt,
        output_format=BatchOutputFormat.VIDEO,
        cleanup_mode=BatchCleanupMode.KEEP,
        swapper_execution=swap_exec,
        enhancer_execution=enh_exec,
        upscaler_execution=up_exec,
    )
    # Batch-only inputs forwarded unchanged.
    assert task.source_path == src
    assert task.target_path == tgt
    assert task.output_format == BatchOutputFormat.VIDEO
    assert task.cleanup_mode == BatchCleanupMode.KEEP
    assert task.swapper_execution == swap_exec
    assert task.enhancer_execution == enh_exec
    assert task.upscaler_execution == up_exec
    # Processor fields sourced from the snapshot (str-Enum → token).
    assert task.swapper_enabled == s.swapper_enabled
    assert task.swapper_model == s.swapper_params.model.value
    assert task.swapper_detector == s.swapper_params.detector.value
    assert task.swapper_occlusion_parser == s.swapper_params.occlusion_parser.value
    assert task.enhancer_model == s.enhancer_params.model.value
    assert task.upscaler_model == s.upscaler_params.model.value
    assert task.video_backend == s.video_backend
    assert task.reader_pool_size == s.reader_pool_size
    assert task.processing_scale == s.processing_scale
    assert task.image_format == s.image_format
    assert task.image_quality == s.image_quality
