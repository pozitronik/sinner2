...
class TestLoadNotifier:
    ...
    def test_notify_load_calls_installed_notifier(self):
        seen: list[str] = []
    ...
    def test_notify_load_noop_without_notifier(self):
        face_analyser.set_load_notifier(None)
        face_analyser._notify_load("nobody listening")  # noqa: SLF001  # no raise

    def test_notify_load_swallows_notifier_errors(self):
        def boom(_msg):
            raise RuntimeError("ui blew up")

        face_analyser.set_load_notifier(boom)
    ...
    def test_first_run_flags_download_then_clears(self, monkeypatch, tmp_path):
        # Pack dir absent → the build downloads; the notifier must see a START
        # message then "" (END), even though insightface itself has no hook.
        ...
        monkeypatch.setattr(insightface.app, "FaceAnalysis", _FakeAnalysis)
        seen: list[str] = []
        face_analyser.set_load_notifier(seen.append)
        reset_shared_face_analysis()
    ...
    def test_present_pack_does_not_flag_download(self, monkeypatch, tmp_path):
        ...
        reset_shared_face_analysis()
        face_analyser._get_shared_face_analysis()  # noqa: SLF001
        assert seen == []  # nothing downloaded → no caption
